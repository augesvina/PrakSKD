...
#membuat enkripsi dan dekripsi menggunakan playfair cipher
key=input("Enter key")
key=key.replace(" ", "")
key=key.upper()
...
result=list()
for c in key: # membuat fungsi untuk menyimpan kunci
   if c not in result:
       if c=='J':
           result.append('I')
       else:
           result.append(c)
flag=0
for i in range(65,91): # fungsi untuk menyimpan karakter lain
   if chr(i) not in result:
       if i==73 and chr(74) not in result:
           ...
           result.append(chr(i))
k=0
my_matrix=matrix(5,5,0) # membuat index inisialisasi matriks
for i in range(0,5): # membuat matriks
   for j in range(0,5):
       my_matrix[i][j]=result[k]
       k+=1

def locindex(c): # mengambil lokasi setiap karakter
   loc=list()
   if c=='J':
       c='I'
